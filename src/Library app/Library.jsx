  ...
  useEffect(() => {
    const fetchbooks = async () => {
      try {
        ...
        setbooks(response.data.playlists);
      } catch (error) {
  ...
  return (
    <div>
      <div className="flex justify-between bg-blue-400 h-16 px-8 items-center">
        <Link to="/library">
          <h1 className="flex gap-2 text-3xl font-bold font-serif items-center">
              ...
              <div className="absolute right-0 mt-2 w-32 bg-black text-white border rounded-md shadow-lg z-10">
                <ul>
                  <li className="px-4 py-2 hover:bg-gray-600 cursor-pointer">
                    <button onClick={handlecategory} value="Adventure">
                      Adventure
                    </button>
                  </li>
                  <li className="px-4 py-2 hover:bg-gray-600 cursor-pointer">
                    <button onClick={handlecategory} value="Science">
                      Science
                    </button>
                  </li>
                  <li className="px-4 py-2 hover:bg-gray-600 cursor-pointer">
                    <button onClick={handlecategory} value="Horror">
                      Horror
                    </button>
                  </li>
                  <li className="px-4 py-2 hover:bg-gray-600 cursor-pointer">
                    <button onClick={handlecategory} value="Cartoon">
                      Cartoon
                    </button>
                  </li>
                  <li className="px-4 py-2 hover:bg-gray-600 cursor-pointer">
                    <button onClick={handlecategory} value="Romance">
                      Romance
                    </button>
                  </li>
                  <li className="px-4 py-2 hover:bg-gray-600 cursor-pointer">
                    <button onClick={handlecategory} value="Fiction">
                      Fiction
                    </button>
                  </li>
                  <li className="px-4 py-2 hover:bg-gray-600 cursor-pointer">
                    <button onClick={handlecategory} value="Dram">
                      Drama
                    </button>
                  </li>
                  <li className="px-4 py-2 bg-red-600 hover:bg-gray-600 cursor-pointer">
          ...
          .map(
            (item) =>
              item.title.toLowerCase().includes(Search.toLowerCase()) && (
                <div
                  key={item.id}
                  className="p-4 shadow-md hover:shadow-xl transition-shadow duration-300 rounded-xl"
                >
            ...
            <h1 className="mb-2 font-bold">Copies sold : {item.copies_sold}</h1> */}
                  <h1>
                    <button
                      onClick={() => dispatch(mylibrary(item))}
                      className="btn rounded-md bg-blue-400 px-4 py-2 font-semibold"
                    >
                      Purchase
                    </button>
                  </h1>
